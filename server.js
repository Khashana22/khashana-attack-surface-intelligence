...
const server = http.createServer(async (req,res) => {
  ...
  try {
    ...
    if (req.method==='GET' && url.pathname==='/api/findings') return json(res,200,findings);
    if (req.method==='GET' && url.pathname==='/api/health') return json(res,200,{status:'ok',mode:'synthetic-local-demo',time:now});
    if (req.method==='POST' && url.pathname==='/api/scans') { const data=await body(req); if (!String(data.target||'').endsWith('.local')) return json(res,403,{error:'Local-lab-only policy: targets must end in .local. No scan was performed.'}); return json(res,202,{id:crypto.randomUUID(),status:'completed',target:data.target,message:'Synthetic local-lab fixture normalized. No network scan was executed.'}); }
    if (req.method==='POST' && /^\/api\/findings\/[^/]+\/review$/.test(url.pathname)) { const id=url.pathname.split('/')[3], data=await body(req), finding=findings.find(f=>f.id===id); if(!finding) return json(res,404,{error:'Finding not found'}); const allowed=['Validated','False Positive','Needs Validation','Accepted Risk','Remediated','Retest Required','Closed']; if(data.status && !allowed.includes(data.status)) return json(res,400,{error:'Invalid finding state'}); finding.status=data.status||finding.status; finding.notes=String(data.notes||'').slice(0,4000); finding.validation=String(data.validation||'').slice(0,4000); reviews[id]={...data,updatedAt:new Date().toISOString()}; return json(res,200,{finding,review:reviews[id]}); }
    if (req.method==='GET' && url.pathname==='/api/reports/executive.pdf') { const d=dashboard(); const report=pdf(`Executive summary: ${d.metrics.assets} assets observed; ${d.metrics.openFindings} open findings; highest priority: ${findings[1].id} on dev.northstar.local needs researcher validation.`); res.writeHead(200,{'Content-Type':'application/pdf','Content-Disposition':'attachment; filename="northstar-executive-report.pdf"','Content-Length':report.length}); return res.end(report); }
